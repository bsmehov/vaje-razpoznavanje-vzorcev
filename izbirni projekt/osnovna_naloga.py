...
def racunanje_rojev(evklidove_razdalje,euc_za_rac,tocke1,stevec,N,seznam_razdalj,tocke,a,stevilo_vzorcev):
    ...
    while(N > 1):
        ...
        for i in range(len(evklidove_razdalje)):         
            if(a == 3):
                imenovalec = (stevilo_vzorcev[i] + stevilo_vzorcev[tocke1[stevec+1]] + stevilo_vzorcev[tocke1[stevec]])
                a_i = (stevilo_vzorcev[i] + stevilo_vzorcev[tocke1[stevec]])/imenovalec
                a_j = (stevilo_vzorcev[i] + stevilo_vzorcev[tocke1[stevec+1]])/imenovalec
                b = -stevilo_vzorcev[i]/imenovalec
                c = 0

            razdalja = a_i*euc_za_rac[tocke1[stevec]][i] + a_j*euc_za_rac[tocke1[stevec+1]][i] + b*euc_za_rac[tocke1[stevec+1]][tocke1[stevec]] + c*abs(euc_za_rac[tocke1[stevec]][i] - euc_za_rac[tocke1[stevec+1]][i])
...
if __name__ == "__main__":
    ...
    evklidove_razdalje = np.where(evklidove_razdalje == 0, 1000, evklidove_razdalje)
    ...
    euc_za_rac = np.where(euc_za_rac == 0, 1000, euc_za_rac)
    ...
    stevec2 = 0
    stevilo_vzorcev1 = [1, 1, 1, 1, 1, 1, 1, 1]
    ...
    df = pd.DataFrame(evklidove_razdalje_za_Q)
    df.to_excel(excel_writer = "C:/Users/bsmeh/Documents/osnovna_naloga_kofeneticna_matrika_1.xlsx")
    df = pd.DataFrame(evklidove_razdalje_za_Q)
    df.to_excel(excel_writer = "C:/Users/bsmeh/Documents/osnovna_naloga_kofeneticna_matrika_2.xlsx")
    df = pd.DataFrame(evklidove_razdalje_za_Q)
    df.to_excel(excel_writer = "C:/Users/bsmeh/Documents/osnovna_naloga_kofeneticna_matrika_3.xlsx")
